...
function Player:checkObjectCollisions()
    ...
    love.graphics.draw(gTextures['keys-and-locks'], gFrames['keys-and-locks'][5], 0,0)
    for k, object in pairs(self.level.objects) do
        if object:collides(self) then
          if object.solid then
            local l = object.onConsume(self)
                if l then
                  table.remove(self.level.objects, k)
                end
                table.insert(collidedObjects, object)
            --elseif object.isLockBlock then-- and isKeySpawned 
              --  local l = object.onConsume(self)
               -- if l then
                 -- table.remove(self.level.objects, k)
                --end
            elseif object.consumable then--object.isLockBlock == false and object.consumable then
